...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot, image_dataroot, ratio, adaptive=False, opt=None):
        ...
        is_exist = os.path.exists('data4VE/C_'+name+'_top20_densecaption_tokenizer_ids.pkl')
        if not is_exist:
            self.entries = _load_dataset(dataroot, name, self.label2ans, ratio)
            self.tokenize(max_length=18, condi_ans_num=self.condi_ans_num)
            self.tensorize(name)
        else:
            fp = open('data4VE/C_'+name+"_top20_densecaption_tokenizer_ids.pkl","rb+")
            self.entries = pickle.load(fp)
    def tokenize(self, max_length=18, condi_ans_num=5):
    ...
    def tensorize(self, name):
        ...
        with open('data4VE/C_'+name+'_top20_densecaption_tokenizer_ids.pkl', 'wb') as f:
